<template>
    <div class="popup popup1 md-content ">
        <form id="addToken" method="#">
            <h1>Add Token</h1>
            <div class="row">
                <input class="add_or_update_token" type="hidden" value="0"/>
                <!-- 0 = add , 1 = update -->
                <p v-if="contractAddressError" class="error-message tokenHash-error">{{contractAddressError}}</p>
                <span class="input input--nao">
                    <input type="text" name="token_contract_address" v-model="contractAddress" @focus="handleFoucs" @change="handleContractAdd"
...
<script>
    ...
    import { isMobileDevice } from '../../AccountsData/commonFunc';
    ...
    export default {
        ...
        methods: {
            handleContractAdd(){
                let that = this;
                let token_hash = this.contractAddress;
                // console.log(this.contractAddress,"this.contractAddress",tokenInterface )
                // console.log(web3,"web3")
                var TokenContract = new web3.eth.Contract(tokenInterface, token_hash);
                console.log(TokenContract,"TokenContract")
                try{
                    console.log("try TokenContract", TokenContract.methods.name()._methods)
                    TokenContract.methods.name.call(function(err ,res){
                        ...
                        that.tokenName = res;
                        console.log(res,"try tokensymbol")
                    });
                }catch(e){
                    that.tokenName = '';
                }
            ...
            handleAddToken(e){
                e.preventDefault();
                if(this.contractAddress && this.tokenName && this.tokenType && this.tokensymbol && this.decimalplaces){
                    let token = db.get('tokens').find({ token_address: this.contractAddress }).value();
                    if(this.contractAddress.length < 5){
                        this.contractAddressError = 'Hash Address is required';
                    }else{
                        if((!ethereum_address.isAddress(this.contractAddress))){
                            this.contractAddressError = 'Invalid Hash Address';
                        }else {
